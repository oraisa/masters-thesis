...
if __name__ == "__main__":
    ...
    m = 100
    d = 2
    sample1 = np.random.randn(n * d).reshape((m, d))# + np.ones((100, 1))
    sample2 = np.random.randn(m * d).reshape((m, d))
    print("MMD: {}".format(np.sqrt(np.abs(mmd(sample1, sample2)))))
    mmd_res = mmd(sample1, sample2)
    ...
    # res3 = timeit.timeit(lambda: mmd(sample1, sample2), number=1)
    # print(res1)
    # print(res2)
    # print(res3)
