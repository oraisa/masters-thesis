...
\emph{Differential privacy} (DP)~\cite{DwR14} is a property of
...
differential privacy (ADP)~\cite{DwR14}.
...
widely used method to obtain such samples~\cite{BDA}.
...
distribution \(q(\cdot \mid \theta_{i-1})\)~\cite{BDA}, where \(\theta_{i-1}\) is the
...
test as above. See Algorithm~\ref{DP_barker_algo} for the DP Barker
algorithm\footnote{
