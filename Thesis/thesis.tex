...
(GRMH)~\cite{YildirimE19}.
In GRMH, proposals change only one dimension, as above. Additionally, a direction
...
\begin{theorem}\label{dp_hmc_theorem_zcdp}
  ...
  \]
  iterations using \(\sigma_{l}\) as log likelihood ratio noise variance,
  \(\sigma_{g}\) as gradient noise variance, clipping log likelihood ratios
...
DP penalty with subsampling uses Theorem~\ref{dp_penalty_minibatch_theorem}
