...
\section{Comparing Privacy Accounting Methods}
...
\chapter{Proof of Lemma X}
\begin{lemma}\label{clip_is_almost_everywhere_differentiable}
  Let \(f\colon \R^{d}\to \R^{d}\) be continuously differentiable in an
  open set \(U\) with
  \(\det J_{f}(x) \neq 0\) for all \(x\in U\).
  If \(U^{C}\) has zero measure, the set \(A\subset \R^{d}\)
  where \(\clip_{b} \circ f\) is not differentiable has zero measure.
\end{lemma}
\begin{proof}
  \[
  ...
  in \(d\)-dimensional space. Because \(A\cap U \subset S\), \(A\cap U\)
  also has zero measure. Since \(A = (A\cap U) \cup (A\cap U^{C})\),
  \(A\) also has zero measure.
...
Lemma~\ref{clip_is_almost_everywhere_differentiable} for \(U\) are that
\(U\) is twice continuously differentiable and
\[
  H_{U}(\theta) \neq 0
\]
where \(H_{U}\) is the Hessian matrix of \(U\).
\begin{lemma}
...
This means that the set \(U\) where \(\det H_{U}(x, y) \neq 0\) is open and
\(U^{C}\) has zero measure.
\end{appendices}
