...
Definition~\ref{reversible_definition} is central to the results in
Chapter~\ref{hmc_chapter}, so looking into it in detail is warranted.
Let \((E, \mathcal{E})\) be a measurable space and let,
\[
  \nu_{1}(A\times B) = \int_{A}\mu(\dx a)\int_{B}q(a, \dx b).
\]
where \(A, B\in \mathcal{E}\). By Theorem~\ref{product_measure_theorem},
this defines a unique measure \(\nu_{1}\) on \((E, \mathcal{E})^{2}\).
In this way, the equality in Definition~\ref{reversible_definition} can be
seen as an equality between two measures. On the left side is the measure
\(\nu_{1}\), and on the right side is a measure \(\nu_{2}\),
both on \((E, \mathcal{E})^{2}\), where
\(\nu_{2}(A\times B) = \nu_{1}(B\times A)\).
Definition~\ref{reversible_definition} requires that \(\nu_{1} = \nu_{2}\),
in other words, that reversing the order of the pairs in a set does not change the
\(\nu\)-measure of that set.
...
\begin{proof}
  ...
  for any \(A, B, C, D\in \mathcal{B}(\R^{d})\),
    ...
    \\=& \int_{C}m(\dx\theta')\int_{D}m(\dx p')\int_{A\times B}(l_{-}\circ l_{p_{x}})(\theta', p', \dx(\theta, p))
  \end{align*}
