...
\section{DP Penalty}\label{dp_penalty_section}
...
\section{DP Barker}\label{dp_barker_section}
...
but it no longer simulate the correct Hamiltonian dynamics~\cite{CFG14}. This does not affect
...
% Computing the ADP bound directly with
% Theorem \ref{gauss-DP-bounds} is not possible because the gradients and
% log likelihood ratios have different sensitivities and noise levels. However,
% the proof of Theorem~\ref{gauss-DP-bounds}~\cite{Sommer2019} is straightforward
...
models was \(\sigma_{1} = 20\), \(\sigma_{2} = 2.5\) and \(\sigma_{i} = 1\)
...
Publicly available implementations of DP Barker
\footnote{\url{https://github.com/DPBayes/DP-MCMC-NeurIPS2019}}
and the Fourier accountant
\footnote{\url{https://github.com/DPBayes/PLD-Accountant}} were
...
\footnote{\url{https://github.com/oraisa/masters-thesis}}
...
The experiments this section compare the DP MCMC algorithms discussed in this
...
In the non-tempered models, the minibatch algorithms are not are not very useful,
...
Figure~\ref{banana_grad_clipping_fig} shows the fraction of clipped
