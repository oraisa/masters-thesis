...
\begin{algorithm}[H]\label{DP_barker_algo}
    ...
    \Return \((\theta_1, \dotsc, \theta_k)\)
    \caption{
      DP Barker: number of iterations \(k\), initial value \(\theta_{0}\),
...
\begin{algorithm}
    ...
    \For{\(1 \leq i \leq k\)}{
        \(\theta_{i,0} = \theta_{i-1}\)\\
        \(G_{i,0} = g_{b_{g}}(\theta_{i,0}) + \caln(0, \sigma_{g}^{2})\)\\
        \(p_0 = \caln_d(0, M)\)\\
        \For{\(1 \leq j \leq L\)}{
    ...
    where \(\sigma_{l}(\theta, \theta') = 2\tau_{l}\sqrt{n}b_{l}||\theta - \theta'||_{2}\)
    and \(g_{b_{g}}(\theta) = \sum_{x\in X}\clip_{b_{g}}(\nabla \ln p(x\mid \theta))
    + \nabla \ln p(\theta)\).
    \caption{
      ...
      and dataset \(X\) as input. Each \(\caln(\mu, \Sigma)\) represents
      sampling independently from the Gaussian distribution with mean
