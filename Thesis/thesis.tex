...
\begin{algorithm}[H]\label{DP_barker_algo}
    ...
    \For{\(1 \leq i \leq k\)}{
        \For{\(j \in B\)}{
            \(r_j = \ln \frac{p(\theta^*\mid D_j)}{p(\theta_{i-1}\mid D_j)}\)\\
        }
...
\[
    \min\{1, e^{\lambda - \frac{1}{2}(\sigma^2 + \sigma_b^2)}\},
\]
where
\[
\lambda = \frac{nT}{|B|}\sum_{j\in B}
\ln \frac{p(D_j\mid \theta')}{p(D_j \mid \theta)}
...
batch \(B\). Denote
\[
    r_j = \ln \frac{p(D_j\mid \theta')}{p(D_j\mid \theta)},
\]
\[
R = \sum_{x\in B}r_j.
\]
...
\end{lemma}
\begin{proof}
    For datasets \(D \sim D'\), that only differ in one element, denote the 
    common part they have by \(D^*\), and the differing element by \(d\in D\) 
    and \(d'\in D'\)
    \begin{align*}
        \Delta \sigma^2_b &= \sup_{D\sim D'} |\sigma^2_b(D) - \sigma^2_b(D')|
        \\&= \left(\frac{nT}{b}\right)^2 \sup_{D\sim D'}\Big|
        \sum_{d\in D}r^2(d) - \sum_{d\in D'}r^2(d)
        + \frac{1}{b}R^2(D') - \frac{1}{b}R^2(D)\Big|
        \\&= \left(\frac{nT}{b}\right)^2 \sup_{d, d', D^*}\Big|
        r^2(d) - r^2(d')
        + \frac{1}{b}(R(D^*) + r(d'))^2 - \frac{1}{b}(R(D^*) + r(d))^2\Big|
        \\&= \left(\frac{nT}{b}\right)^2 \sup_{d, d', D^*}\Big|
        r^2(d) - r^2(d')
        + \frac{1}{b}(R^2(D^*) + 2R(D^*)r(d') + r^2(d'))
        \\&- \frac{1}{b}(R^2(D^*) + 2R(D^*)r(d) + r^2(d))\Big|
        \\&= \left(\frac{nT}{b}\right)^2 \sup_{d, d', X^*}\Big|
        \left(1 - \frac{1}{b}\right)(r^2(d) - r^2(d'))
        + \frac{2}{b}D(X^*)(r(d') - r(d))\Bigg|
        \\&\leq \left(\frac{nT}{b}\right)^2 \left|1 - \frac{1}{b}\right|
        \sup_{d, d'}\Big|(r^2(d) - r^2(d'))\Big|
        + \frac{2}{b}\left(\frac{nT}{b}\right)^2 \sup_{d, d', D^*}\Big|
        R(D^*)(r(d') - r(d))\Bigg|
        ...
        (b - 1)\sup_{d}|r(d)|.
    \end{align*}
    Plugging the bound \(\sup_d |r(d)| \leq L||\theta - \theta'||_2\) 
