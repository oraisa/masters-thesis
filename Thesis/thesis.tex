...
\begin{proof}
  ...
  \]
    ...
    % \\&= \int_{\R^{d}}q_{1}(c, \dx b)q_{2}(b, \dx a)\mu(\dx c)
...
\begin{proof}
  ...
  \[
    ...
    % \int_{\{p'\mid (\theta', p')\in B \wedge \theta = \theta'\}}\caln(p'\mid -p + f_{x}(\theta), \sigma^{2})\dx p'
    % \begin{cases}
    %   0 & \text{if } \theta \neq \theta'\\
    %   \caln(p'\mid -p + f(\theta), \sigma^{2}) & \text{if } \theta = \theta'
    % \end{cases}
  \]
  Using Lemma~\ref{dirac_measure_reversible_lemma}
    ...
    \\&= \int_{B}\int_{A}\caln(p\mid -p' + f_{x}(\theta'), \sigma^{2})
      \delta_{\theta'}(\dx \theta)\dx p\dx(\theta', p')
    % &= \int_{A}\int_{B}\mu(\theta, p, \dx(\theta', p'))\dx(\theta, p)
    % &= \int_{A}\int_{\{p'\mid (\theta', p'\in B) \wedge \theta' = \theta\}}
    % \caln(p'\mid -p + f_{x}(\theta), \sigma^{2})\dx p'\dx(\theta, p)
    % \\&= \int_{\{(\theta, p, p')\mid (\theta, p)\in A \wedge (\theta', p')\in B \wedge \theta' = \theta\}}
    % \caln(p\mid -p' + f_{x}(\theta), \sigma^{2})\dx(\theta, p, p')
    % \\&= \int_{\{(\theta', p, p')\mid (\theta, p)\in A \wedge (\theta', p')\in B \wedge \theta' = \theta\}}
    % \caln(p\mid -p' + f_{x}(\theta'), \sigma^{2})\dx(\theta', p, p')
    % \\&= \int_{B}\int_{\{p\mid (\theta, p\in A) \wedge \theta' = \theta\}}
    % \caln(p\mid -p' + f_{x}(\theta'), \sigma^{2})\dx p\dx(\theta', p')
    % \\&= \int_{A}\int_{B}I_{\theta}(\theta')\caln(p'\mid -p + f(\theta), \sigma^{2})
    % \dx(\theta', p')\dx(\theta, p)
    % \\&= \int_{B}\int_{A}I_{\theta'}(\theta)\caln(p\mid -p' + f(\theta'), \sigma^{2})
    % \dx(\theta, p)\dx(\theta', p')
    % \\&= \int_{B}\int_{A}\mu(\theta', p', \dx(\theta, p))\dx(\theta', p')
