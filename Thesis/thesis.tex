...
\begin{algorithm}[h]
  \KwIn{Number of iterations \(k\), target distribution \(p\),
    proposal distribution \(q\), initial value \(\theta_{0}\), dataset \(X\)
  }
  \For{\(1 \leq i \leq k\)}{
    denote \(\theta = \theta_{i-1}\)\\
    sample \(\theta' \sim q(\cdot \mid \theta)\)\\
    \(\ln \frac{p(X\mid \theta')}{p(X\mid \theta)} =
    \sum_{x\in X} (\ln p(x \mid \theta') - \ln p(x\mid \theta))
    \)\\
...
\begin{algorithm}[H]\label{max_iterations_algo}
  \SetKwFunction{zcdp}{zcdp}
  \SetKwFunction{adp}{adp}
  \SetKwData{low}{low}
  \SetKwData{high}{high}
  \SetKwData{new}{new}
  \KwIn{\(\epsilon, \delta, \tau\) and \(n\)}
  ...
  \While{\(\adp(\epsilon, \high, \tau, n) < \delta\)}{
    \(\high = \high \cdot 2\) \\
  }
  \While{\(\lfloor \high \rfloor - \lfloor \low \rfloor > 1\)}{
    \tcp{\new does not need to be an integer as \adp can handle a non-integer
  ...
  \eIf{\(\adp(\epsilon, \lfloor \high \rfloor, \tau, n) < \delta\)}{
...
\begin{algorithm}[h]
  \KwIn{Number of iterations \(k\), initial value \(\theta_{0}\), target distribution
  \(p\), proposal distribution \(q\), noise variance \(\sigma^{2}\), data \(X\)}
  ...
  \Return \((\theta_1, \dotsc, \theta_k)\)
  \caption{DP Barker}\label{DP_barker_algo}
\end{algorithm}
...
% Both \(l_{\theta}\) and \(l_{p_{t}}\) preserve measure, as \(l_{\theta}\) in
% unchanged from the non-DP case and the Jacobian of \(l_{p_{t}}\) is
% \[
%   J_{l_{p_{t}}}(\theta, p) =
%   \begin{bmatrix}
%     I_{d\times d} & 0_{d\times d} \\
%     tJ_{g}(\theta) & I_{d\times d}
%   \end{bmatrix},
% \]
...
% does not affect the volume preservation of \(l_{p_{t}}\).
...
% Because \(J_{l_{p_{t}}}(\theta, p)\) is triangular and the diagonal consists
% of only ones, \(\det J_{l_{p_{t}}}(\theta, p) = 1\)
% for all \(\theta\) and \(p\), which means that \(l_{p_{t}}\) preserves volume.
...
\begin{algorithm}
  \KwIn{target distribution \(p\), clip bounds \(b_{l}\) and \(b_{g}\),
    number of iterations \(k\), number of leapfrog steps \(L\), leapfrog step
    size \(\eta\), initial value \(\theta_{0}\), gradient noise variance \(\sigma_{g}^{2}\),
    log-likelihood noise parameter \(\tau_{l}\) ,
    positive-definite mass matrix \(M\) and data \(X\)
  }
  \(\sigma_{l}(\theta, \theta') = 2\tau_{l}\sqrt{n}b_{l}||\theta - \theta'||_{2}\)\\
  \(g_{b_{g}}(\theta) = \sum_{x\in X}\clip_{b_{g}}(\nabla \ln p(x\mid \theta))
  + \nabla \ln p(\theta)\)\\
  \For{\(1 \leq i \leq k\)}{
    \(\theta_{i,0} = \theta_{i-1}\)\\
    sample \(G_{i,0} \sim \caln(g_{b_{g}}(\theta_{i,0}), \sigma_{g}^{2})\)\\
    sample \(p_0 \sim \caln_d(0, M)\)\\
    \For{\(1 \leq j \leq L\)}{
      \(p_{j-\frac{1}{2}} = p_{j-1} + \frac{1}{2}\eta G_{i,j-1} \)\\
      \(\theta_{i,j} = \theta_{i,j-1} + M^{-1}p_{j-\frac{1}{2}}\)\\
      sample \(G_{i,j} \sim \caln(g_{b_{g}}(\theta_{i,j}), \sigma_{g}^{2})\)\\
      \(p_{j} = p_{j-\frac{1}{2}} + \frac{1}{2}\eta G_{i,j} \)\\
    }
    \(r_x = \ln \frac{p(x\mid \theta_{i,L})}{p(x\mid \theta_{i-1})}\)\\
    \(R = \sum_{x\in X}^n \mathrm{clip}_{b_l||\theta_{i-1} - \theta_{i,L}||_2}(r_x)\)\\
    sample \(\xi \sim \caln(0, \sigma_{l}^{2}(\theta_{i-1}, \theta_{i, L}))\)\\
    \(\Delta H = R + \frac{1}{2}p_0^TM^{-1}p_0 - \frac{1}{2}p_L^TM^{-1}p_L
    + \ln \frac{p(\theta_{i,L})}{p(\theta_{i-1})}
    + \xi\)\\
    sample \(u \sim \mathrm{Unif}(0, 1)\)\\
    \eIf{\(u < \Delta H - \frac{1}{2}\sigma_{l}^{2}(\theta_{i-1}, \theta_{i,L})\)}{
      \(\theta_{i} = \theta_{i, L}\)\\
    }
    {
      \(\theta_i = \theta_{i-1}\)\\
...
% \renewcommand{\thetheorem}{\ref{banana_posterior_theorem}}
