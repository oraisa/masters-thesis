...
\keywords{Differential privacy, Metropolis-Hastings, Markov chain Monte Carlo, Hamiltonian Monte Carlo}
\depositeplace{}
...
requires a framework for studying Metropolis-Hastings algorithms
...
continuous proposal distributions.
This section introduces measure theoretic probability theory, in particular
the theory of Markov kernels, that does provide a sufficiently general
...
The subsets of \(\mathcal{P}(E)\) are also called collections to
...
is denoted by \((E, \mathcal{E})^{2}\), and the product or measurable
spaces \((E_{i}, \mathcal{E}_{i})\) for \(1\leq i \leq d\) is denoted by
\(\bigotimes_{i}^{d}\mathcal{E}_{i}\).
...
\begin{lemma}
  ...
  countable partition of \(E\) into sets \(B_{i}\) for \(i\in \N_{+}\)
  of the form
  \(B_{i} = \bigtimes_{j=1}^{d}A_{j}^{i}\) with
  \(A_{i}^{j}\in \mathcal{E}\) for all \(i, j\in \N_{+}\) such that
  \(\mu(B_{i}) < \infty\) and \(\nu(B_{i}) < \infty\). Then \(\mu = \nu\)
  if and only if
  \[
    \mu\left(\bigtimes_{i=1}^{d}A_{i}\right)
    = \nu\left(\bigtimes_{i=1}^{d}A_{i}\right)
  \]
  for all \(A_{j} \in \mathcal{E}\).
\end{lemma}
\begin{proof}
	Clearly, if \(\mu = \nu\),
  \[
  ...
  \(\bigtimes_{i=1}^{d}A_{i}\) form a p-system that generates
  \(\bigotimes_{i=1}^{d}\mathcal{E}\). Denote \((\mu| B)(A) =\mu(A\cap B)\)
  for \(A, B\in \mathcal{E}\).
  Then \(\mu| B\) is a measure on \((E, \mathcal{E})^{d}\)~\cite{Cin11}.
  Now the measures \(\mu|B_{i}\) and \(\nu|B_{i}\) are finite
  and
  \[
    (\mu|B_{i})\left(\bigtimes_{i=1}^{d}A_{i}\right)
    = (\nu|B_{i})\left(\bigtimes_{i=1}^{d}A_{i}\right)
  \]
  for all \(A_{i}\in \mathcal{E}\). By Lemma~\ref{p_system_measure_equality_lemma},
  \(\mu|B_{i} = \nu|B_{i}\) for all \(i\in \N_{+}\). Then
  \[
    \mu(A) = \mu(E\cap A)
...
\begin{proof}
  ...
  \[
    \mu^{2}(A\times B) = \int_{A}\mu(\dx a)\int_{B}\mu(\dx b).
  \]
  ...
  \begin{align*}
    ...
    \\&= \nu_{2}(A\times B\times C\times D).
  \end{align*}
  % \(\nu_{1}\) and \(\nu_{2}\) are measures on \((E, \mathcal{E})^{4}\),
  As \(\mu\) is \(\sigma\)-finite, there is a countable partition \(E_{i}\) of \(E\)
  such that \(\mu(E_{i}) < \infty\) for all \(i\in \N_{+}\). Then
  \(E_{i}\times E_{j}\times E_{k}\times E_{l}\) for \(i, j, k, l\in \N_{+}\) is a
  ...
  \begin{align*}
    ...
    \\&< \infty.
  \end{align*}
  Then Lemma~\ref{reversible_check_product_lemma} shows that
  \(\nu_{1} = \nu_{2}\).
\end{proof}
...
\begin{proof}
  ...
  the third equality uses uses Theorem~\ref{lebesgue_integral_theorem} (5)
  ...
  Thus the claim holds for all \(k\in \N_{+}\) by induction.
\end{proof}
...
\begin{proof}
  ...
  for all \(A, B\in \mathcal{B}(\R^{d})\), holds~\cite{Tie98}\footnote{
    ...
    of measures, which, by Theorem~\ref{product_measure_theorem}, can be
...
\end{lemma}
\begin{proof}
  Denote the Lebesgue measure by \(m\). Because \(f\) preserves
...
implies \(f\) preserving the Lebesgue measure, recall that the measure of a set
\(A\) is given by 
\[
    m(A) = \int_A m(\dx x)\, 1.
\]
...
Because \(J_{l_{p_{x}}}(\theta, p)\) is triangular and the diagonal consists
of only ones, \(\det J_{l_{p_{x}}}(\theta, p) = 1\)
...
\begin{proof}
  ...
  For \(l_{-}\circ l_{p_{x}}\), some care with product measurable spaces is
  required. \(l_{-}\circ l_{p_{x}}\) is a Markov kernel on \(\R^{2d}\),
  so the claim of the reversibility of \(l_{-}\circ l_{p_{x}}\) with respect
  to the Lebesgue measure is a claim about measures in \(\R^{4d}\).
  Because \(\mathcal{B}(\R^{4d}) = \mathcal{B}(\R^{d})\otimes\mathcal{B}(\R^{d})
  \otimes\mathcal{B}(\R^{d})\otimes\mathcal{B}(\R^{d})\), by
  Lemma~\ref{reversible_check_product_lemma} it is sufficient to show that
  \begin{align*}
    &\int_{A}m(\dx \theta)\int_{B}m(\dx p)
    \int_{C\times D}(l_{-}\circ l_{p_{x}})(\theta, p, \dx (\theta', p'))
    \\= &\int_{C}m(\dx \theta')\int_{D}m(\dx p')
    \int_{A\times B}(l_{-}\circ l_{p_{x}})(\theta', p', \dx (\theta, p))
  \end{align*}
  Denote \(f_{x}(\theta) = x\eta g(\theta)\) and
  \(\sigma^{2} = x^{2}\eta^{2}\sigma_{g}^{2}\). Denote the density function
  of the \(d\)-dimensional normal distribution with mean \(\mu\) and covariance
  \(\Sigma\) by \(\caln(\cdot \mid \mu, \Sigma)\).
  Then
  \begin{align*}
    \int_{C\times D}(l_{-}\circ l_{p_{x}})(\theta, p, \dx(\theta', p'))
    &= \int_{C}\delta_{\theta}(\dx \theta')
      \int_{D}m(\dx p')\caln_{d}(p'\mid -p + f_{x}(\theta), \sigma^{2}I)
      \\&= 1_{C}(\theta)\int_{D}m(\dx p')\caln_{d}(p'\mid -p + f_{x}(\theta), \sigma^{2}I)
  \end{align*}
  for \(C, D\in \mathcal{B}(\R^{d})\). The second equality follows from
  Lemma~\ref{dirac_evaluation_lemma}.
  The normal distribution has a shift-invariance property: for any \(x, y\in \R^{d}\)
  \[
    \caln(x\mid \mu, \Sigma) = \caln(x + y\mid \mu + y, \Sigma),
  \]
  which is a consequence of the fact that the density of the normal distribution
  only depends on \(x\) and \(\mu\) through \(x - \mu\).
  Then,
  for any \(A, B, C, D\in \mathcal{B}(\R^{d})\),
	\begin{align*}
    &\int_{A}m(\dx\theta)\int_{B}m(\dx p)\int_{C\times D}
      (l_{-}\circ l_{p_{x}})(\theta, p, \dx(\theta', p'))
    \\=& \int_{A}m(\dx \theta)\int_{B}m(\dx p)\int_{C}\delta_{\theta}(\dx \theta')
         \int_{D}m(\dx p') \caln_{d}(p'\mid -p + f_{x}(\theta), \sigma^{2}I)
    \\=& \int_{A}m(\dx \theta)\int_{C}\delta_{\theta}(\dx \theta')\int_{B}m(\dx p)
         \int_{D}m(\dx p') \caln_{d}(p'\mid -p + f_{x}(\theta), \sigma^{2}I)
    \\=& \int_{C}m(\dx \theta')\int_{A}\delta_{\theta'}(\dx \theta)\int_{B}m(\dx p)
         \int_{D}m(\dx p') \caln_{d}(p'\mid -p + f_{x}(\theta), \sigma^{2}I)
    \\=& \int_{C}m(\dx \theta')\int_{A}\delta_{\theta'}(\dx \theta)\int_{B}m(\dx p)
         \int_{D}m(\dx p') \caln_{d}(p\mid -p' + f_{x}(\theta), \sigma^{2}I)
    \\=& \int_{C}m(\dx \theta')1_{A}(\theta')\int_{B}m(\dx p)
         \int_{D}m(\dx p') \caln_{d}(p\mid -p' + f_{x}(\theta'), \sigma^{2}I)
    \\=& \int_{C}m(\dx \theta')\int_{D}m(\dx p')1_{A}(\theta')
         \int_{B}m(\dx p)\caln_{d}(p\mid -p' + f_{x}(\theta'), \sigma^{2}I)
    \\=& \int_{C}m(\dx\theta')\int_{D}m(\dx p')
         \int_{A\times B}(l_{-}\circ l_{p_{x}})(\theta', p', \dx(\theta, p))
  \end{align*}
  % \begin{align*}
  %   &\int_{\R^{d}}m(\dx\theta)\int_{\R^{d}}m(\dx p)\int_{\R^{d}\times \R^{d}}
  %     (l_{-}\circ l_{p_{x}})(\theta, p, \dx(\theta', p'))f(\theta, p, \theta', p')
  %   \\=& \int_{\R^{d}}m(\dx \theta)\int_{\R^{d}}m(\dx p)\int_{\R^{d}}\delta_{\theta}(\dx \theta')
  %        \int_{\R^{d}}m(\dx p') \caln_{d}(p'\mid -p + f_{x}(\theta), \sigma^{2}I)f(\theta, p, \theta', p')
  %   \\=& \int_{\R^{d}}m(\dx \theta)\int_{\R^{d}}\delta_{\theta}(\dx \theta')\int_{\R^{d}}m(\dx p)
  %        \int_{\R^{d}}m(\dx p') \caln_{d}(p'\mid -p + f_{x}(\theta), \sigma^{2}I)f(\theta, p, \theta', p')
  %   \\=& \int_{\R^{d}}m(\dx \theta')\int_{\R^{d}}\delta_{\theta'}(\dx \theta)\int_{\R^{d}}m(\dx p)
  %        \int_{\R^{d}}m(\dx p') \caln_{d}(p'\mid -p + f_{x}(\theta), \sigma^{2}I)f(\theta, p, \theta', p')
  %   \\=& \int_{\R^{d}}m(\dx \theta')\int_{\R^{d}}\delta_{\theta'}(\dx \theta)\int_{\R^{d}}m(\dx p)
  %        \int_{\R^{d}}m(\dx p') \caln_{d}(p\mid -p' + f_{x}(\theta), \sigma^{2}I)f(\theta, p, \theta', p')
  %   \\=& \int_{\R^{d}}m(\dx \theta')1_{\R^{d}}(\theta')\int_{\R^{d}}m(\dx p)
  %        \int_{\R^{d}}m(\dx p') \caln_{d}(p\mid -p' + f_{x}(\theta'), \sigma^{2}I)f(\theta, p, \theta', p')
  %   \\=& \int_{\R^{d}}m(\dx \theta')\int_{\R^{d}}m(\dx p')1_{A}(\theta')
  %        \int_{\R^{d}}m(\dx p)\caln_{d}(p\mid -p' + f_{x}(\theta'), \sigma^{2}I)f(\theta, p, \theta', p')
  %   \\=& \int_{\R^{d}}m(\dx\theta')\int_{\R^{d}}m(\dx p')
  %        \int_{\R^{d}\times \R^{d}}(l_{-}\circ l_{p_{x}})(\theta', p', \dx(\theta, p))f(\theta, p, \theta', p')
  % \end{align*}
  where the second equality uses Theorem~\ref{lebesgue_integral_theorem} (5)
  to change the order of integration,
  the third uses Lemma~\ref{reversible_integration_lemma} on the two leftmost
  integrals,
  the fourth uses the shift-invariance of the normal distribution,
  the fifth uses Lemma~\ref{dirac_evaluation_lemma} and
  the sixth uses Theorem~\ref{lebesgue_integral_theorem} (5) to rearrange the
  term in the integrals.
\end{proof}
...
\end{theorem}
\begin{proof}
Consider the last four updates of the proposal,
\[
