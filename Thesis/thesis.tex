...
the Markov kernels, that do provide a sufficiently general
...
\begin{proof}
  ...
  \(A\in \bigotimes_{j=1}^{d}\mathcal{E}^{d}\), the same holds for \(\nu|B_{i}\)
  and
  \begin{align*}
...
\begin{align*}
  p_{i+1/2} &= p_{i} - \frac{\eta}{2}(g(\theta_{i}) + \xi) \\
  \theta_{i+1} &= \theta_{i} + \eta M^{-1}p_{i+1/2} \\
  p_{i+1} &= p_{i+1/2} - \frac{\eta}{2}(g(\theta_{i+1}) + \xi)),
\end{align*}
where
\(\xi \sim \caln_{d}(0, \sigma_{g}^{2}I)\) and
\[
...
\]
where
\(\xi \sim \caln_{d}(0, \sigma_{g}^2I)\), \(t_{1} = \eta\) and \(t_{2} = \frac{\eta}{2}\).
...
\begin{lemma}\label{leapfrog_step_reversible_lemma}
  \(l_{\theta}\circ l_{-}\) and \(l_{-}\circ l_{p_{t}}\) for any \(x\in \R\)
