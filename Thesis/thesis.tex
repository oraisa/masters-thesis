...
\begin{definition}
  ...
  % A measure \(\mu\) with \(\mu(E) = 1\) is called a probability measure.
\end{definition}
...
\begin{theorem}
  Useful properties of the Lebesgue integral:
    ...
    % \item For \(f_{1}, f_{2}, \dotsc \in M_{+}(\R_{d})\) where \(f_{i} \geq f_{j}\)
    %       when \(i \geq j\), \(\lim_{i\to\infty}L(f_{i}) = L(\lim_{i\to\infty}f_{i})\).
  ...
  \end{enumerate}
\end{theorem}
% Measure theory is a mathematical theory that seeks to generalise the
% notions of length, areas, and volumes of subsets of \(\R\), \(\R^{2}\)
% and \(\R^{3}\) to high dimensional spaces,
% and unify them with other concepts that behave similarly, such as counts of
% objects and probabilities.

% The Lebesgue measure defines the notions of
% length, area and volume in 1-, 2- and 3-dimensional spaces, and generalises
% them to \(\R^{d}\). It turns out that even assigning a consistent length to all
% subsets of \(\R\) is impossible, but there is a subset of subsets of \(\R\)
% where the assignment of length is possible, the so-called Lebesgue-measurable
% sets. The same holds true for \(\R^{d}\): it is not possible to measure all
% subsets, only the Lebesgue measurable sets. The Lebesgue measure is characterised
% by the following properties:
% \begin{enumerate}
%   \item \(\lambda(\emptyset) = 0\)
%   \item Countable additivity: For disjoint Lebesgue-measurable
%         \(A_{1}, A_{2}, \dotsc\),
%         \(\lambda(\bigcup_{i=1}^{\infty} A_{i}) = \sum_{i=1}^{\infty}\lambda(A_{i})\).
% 	\item Translation invariance: \(\lambda(\{a + x\mid a\in A\}) = \lambda(A)\) for
%         any \(x\in \R^{d}\) and Lebesgue-measurable \(A\subset \R^{d}\).
%   \item \(\lambda([0, 1]^{d}) = 1\).
% \end{enumerate}

% Properties (3) and (4) of the Lebesgue measure are due to the requirement that
% Lebesgue measure must represent lengths in \(\R\), but properties (1) and (2)
% hold in very different settings, some examples being probabilities and the
% sizes of finite sets, so they are taken as the basis when defining general
% measures on arbitrary sets. The issue of measurability from the Lebesgue measure
% is also present in more general measures,
